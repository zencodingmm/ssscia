...
const LoanSubmittion = function(loansubmittion) {
  ...
  this.userid = loansubmittion.userid,
  this.memberid = loansubmittion.memberid,
  this.loan_submit_date = loansubmittion.loan_submit_date;
...
LoanSubmittion.create = (newLoanSubmittion, result) => {
  sql.query("INSERT INTO tbl_loan_submittion SET ?", newLoanSubmittion, (err, res) => {
    ...
    console.log("created loabsubmittion: ", { id: res.insertId, ...newLoanSubmittion });
...
LoanSubmittion.findById = (id, result) => {
  sql.query(`SELECT * FROM tbl_loan_submittion WHERE id = ${id}`, (err, res) => {
    ...
    if (res.length) {
      console.log("found loabsubmittion: ", res[0]);
      result(null, res[0]);
      return;
    }
...
LoanSubmittion.findByMemberId = (id, result) => {
  sql.query(`SELECT * FROM tbl_loan_submittion WHERE memberid = '${id}'`, (err, res) => {
    ...
    if (res.length) {
      console.log("found loabsubmittion: ", res);
      result(null, res);
      return;
    }
...
LoanSubmittion.getAll = (loan_amount, result) => {
  ...
  sql.query(query, (err, res) => {
    ...
    console.log("loansubmittion: ", res);
    result(null, res);
  });
};
...
LoanSubmittion.updateById = (id, loansubmittion, result) => {
  sql.query(
    ...
    [loansubmittion.loan_submit_date, loansubmittion.loan_amount, loansubmittion.land_value_and_acres_to_be_insured, loansubmittion.is_apply_person, loansubmittion.relationship_with_supporters, loansubmittion.is_current_crop_cultivation, loansubmittion.is_taken_loan, loansubmittion.proof_of_repayment, loansubmittion.is_health_and_chronic_disease, loansubmittion.supporter_one_name, loansubmittion.supporter_one_nrc, loansubmittion.supporter_one_dob, loansubmittion.supporter_one_age, loansubmittion.supporter_one_place, loansubmittion.supporter_one_phone, loansubmittion.supporter_two_name, loansubmittion.supporter_two_nrc, loansubmittion.supporter_two_dob, loansubmittion.supporter_two_place, loansubmittion.supporter_two_phone, loansubmittion.health_supporter, loansubmittion.photograph_of_land_to_be_insured, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
      ...
      console.log("updated loansubmittion: ", { id: id, ...loansubmittion });
...
LoanSubmittion.remove = (id, result) => {
  sql.query("DELETE FROM tbl_loan_submittion WHERE id = ?", id, (err, res) => {
    ...
    console.log("deleted loansubmittion with id: ", id);
    result(null, res);
  });
};
