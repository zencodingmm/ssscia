...
Members.findById = (id, result) => {
  sql.query(`SELECT * FROM tbl_members WHERE id = ${id}`, (err, res) => {
    ...
    if (res.length) {
      console.log("found Members: ", res[0]);
      result(null, res[0]);
      return;
    }
...
Members.updateById = (id, members, result) => {
  sql.query(
    ...
    [members.member_id, members.userid, members.first_name, members.last_name, members.father_name, members.dob, members.education, members.ethnicity, members.religion, members.nrc, members.company_name, members.company_address, members.corn_business_life, members.home_address, members.email_address, members.photo, members.home_no, members.home_street, members.home_quater, members.home_village, members.home_township, members.home_division_state, members.number_of_siblings, members.phone_no, members.bank_acc_no, members.bank_name, members.payment_app_number, members.payment_app_type, members.nrc_f_photo, members.nrc_b_photo, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
...
Members.remove = (id, result) => {
  sql.query("DELETE FROM tbl_members WHERE id = ?", id, (err, res) => {
    ...
    console.log("deleted members with id: ", id);
    result(null, res);
  });
};
