...
Posts.updateById = (id, posts, result) => {
	sql.query('UPDATE tbl_posts SET description = ?, cat_type = ?, category_id = ?, links = ?, images = ?, title = ? WHERE id = ?', [posts.description, posts.cat_type, posts.category_id, posts.links, posts.images, posts.title, id], (err, res) => {
		if (err) {
			console.log('error: ', err);
			result(null, err);
			return;
