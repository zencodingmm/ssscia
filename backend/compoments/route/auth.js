...
router.post('/register', (req, res) => {
	...
	sendVerificationEmail(users.email, verificationToken)
		...
		.catch(() => res.status(500).send({ message: 'Some error occurred while created the Users.' }));
});
...
router.post('/login', (req, res) => {
	Users.findByEmailAndPassword({ email: req.body.email, password: req.body.password }, (err, data) => {
		if (err) {
			res.status(500).send({
				message: err.message || 'Invalid email and password!',
			});
		} else {
