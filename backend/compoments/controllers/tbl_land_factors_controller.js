...
exports.create = (req, res) => {
	// Create a Tutorial
	const landfactors = new LandFactors({
		userid: req.body.userid,
		id: req.body.id,
		member_id: req.body.member_id,
		number_of_map: req.body.number_of_map,
		own: req.body.own,
		rent: req.body.rent,
		acreage: req.body.acreage,
		pitch_no: req.body.pitch_no,
		ownership_no: req.body.ownership_no,
		land_type: req.body.land_type,
		acres_of_land_owned: req.body.acres_of_land_owned,
		acres_of_land_ground: req.body.acres_of_land_ground,
		form_seven_form_onehundredfive_acres_of_contract: req.body.form_seven_form_onehundredfive_acres_of_contract,
		...
		names_of_neighbor_of_farm_south: req.body.names_of_neighbor_of_farm_south,
		names_of_neighbor_of_farm_north: req.body.names_of_neighbor_of_farm_north,
		number_of_family_members_by_grandparents: req.body.number_of_family_members_by_grandparents,
		apply_family_one_name: req.body.apply_family_one_name,
		apply_family_one_nrc: req.body.apply_family_one_nrc,
		apply_family_two_name: req.body.apply_family_two_name,
		apply_family_two_nrc: req.body.apply_family_two_nrc,
		time_value: req.body.time_value,
		photo_form_seven: `${req.files.photo_form_seven ? req.files.photo_form_seven[0].filename : ''}`,
		photo_form_onehundredfive: `${req.files.photo_form_onehundredfive ? req.files.photo_form_onehundredfive[0].filename : ''}`,
		photo_contract: `${req.files.photo_contract ? req.files.photo_contract[0].filename : ''}`,
		photo_ancestral_property: `${req.files.photo_ancestral_property ? req.files.photo_ancestral_property[0].filename : ''}`,
		photo_other: `${req.files.photo_other ? req.files.photo_other[0].filename : ''}`,
		photo_household_chart: `${req.files.photo_household_chart ? req.files.photo_household_chart[0].filename : ''}`,
	});
...
exports.findAll = (req, res) => {
	...
	LandFactors.getAll(number_of_map, (err, data) => {
		if (err)
			res.status(500).send({
				message: err.message || 'Some error occurred while retrieving landfactors.',
			});
		else res.send(data);
	});
};
...
exports.update = (req, res) => {
	...
	LandFactors.updateById(req.params.id, landfactors, (err, data) => {
		if (err) {
			if (err.kind === 'not_found') {
				res.status(404).send({
