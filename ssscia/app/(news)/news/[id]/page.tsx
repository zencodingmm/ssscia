	...
	return (
		<div className="w-full px-0 md:px-10 lg:px-20 hoverflow-hidden flex flex-col lg:flex-row gap-6">
			<div className="w-full lg:w-8/12 shadow-xl rounded-md p-4 md:p-8 overflow-y-auto h-screen">
				<img src={`http://localhost:4000/uploads/posts/${data.images}`} className="w-full h-[450px] object-cover" />
				...
				<div dangerouslySetInnerHTML={{ __html: data.description }} className="text-justify my-2" />
				<a href={data.links} className="text-sm text-blue-500">
					{data.links}
				</a>
			</div>
						...
						<div className="space-y-1 overflow-y-auto h-screen border-3">
							{recentNews.length == 0 ? (
								...
								recentNews.map((n: any, index: number) => {
									return (
										<div key={index} className="flex items-start gap-2 border p-2 w-full">
											<Image src={`http://localhost:4000/uploads/posts/${n.images}`} width={100} height={100} alt="local news img" />
